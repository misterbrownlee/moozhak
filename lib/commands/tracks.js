...
export function parseTracksArgs(args, defaultType) {
  if (args.length === 0) {
    return {
      ...
      hint: 'Usage: tracks <id> or tracks <master|release> <id>',
    };
  }
  ...
  if (type !== 'master' && type !== 'release') {
    return {
      ...
      hint: 'Valid types: master, release',
    };
  }
...
export const tracksCommand = {
  ...
  handler: async (args, ctx) => {
    const { type, id, error, hint } = parseTracksArgs(
      args,
      ctx.sessionFlags.tracks_type,
    );
    if (error) {
      log.error(error);
...
export async function handleTracks(db, tracksSearchType, id, flags) {
  ...
  log.success(
    `Found: ${tracksSearchType} #${numId} - ${artists} - ${title}${year ? ` (${year})` : ''}`,
  );
  log.info(`See: ${url}`);
  ...
  if (tracklist.length === 0) {
    ...
    writeTracksOutput(
      formatTracksForFile(tracklist, outputFormat),
      numId,
      outputFormat,
      artists,
      title,
...
function displayTracklist(tracklist, format) {
  ...
  if (format === 'human') {
    ...
    for (const line of getFormatHeader(format)) {
      log.plain(line);
    }
  }
