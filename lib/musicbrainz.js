 ...
 */
export async function lookupByDiscogsRelease(discogsReleaseId, verbose = false) {
 ...
 * // }
 */
export async function getIsrcsForDiscogsRelease(discogsReleaseId, verbose = false) {
...
export async function findTrackIsrcs(artist, title, verbose = false) {
  ...
  const searchResults = await searchRecordings(artist, title, { limit: 5, verbose });

  if (!searchResults || searchResults.error || !searchResults.recordings?.length) {
    return {
      found: false,
  ...
  const bestMatch = searchResults.recordings.find((rec) => {
    const recArtist = rec['artist-credit']?.[0]?.artist?.name?.toLowerCase() || '';
