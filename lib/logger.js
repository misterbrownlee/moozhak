...
export const log = {
  ...
  /** Red error messages */
  error: (...args) => console.error(ansis.red(' ‼️'), ansis.red(...args.map(String))),
  
  /** Yellow warning messages */
  warn: (...args) => console.warn(ansis.yellow(' !!'), ansis.yellow(...args.map(String))),
  
  /** Dim debug/verbose messages */
  debug: (...args) => console.log(ansis.dim(' ○'), ansis.dim(...args.map(String))),
  ...
  divider: (addPadding = false) => { 
    ...
    console.log(ansis.fg(2)`${pad}>>=================================================================<<${pad}\n`);
  }
};
