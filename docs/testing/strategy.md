...
- For routes that resolve **GetSongBPM** or **Discogs** credentials from headers + DB, use [tests/web/testCredentials.js](../../tests/web/testCredentials.js) `withMoozhakCredentials(request)` so supertest sends the same literals as [web/lib/credentialHeaders.js](../../web/lib/credentialHeaders.js) (`x-moozhak-discogs-token`, etc.). Alternatively `PUT /api/settings` in setup to seed `app_settings`, then call routes without headers.
